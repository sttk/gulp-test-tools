...
var istanbulPath = findIstanbulPath();
var coveragePath = findCoveragePath();
...
module.exports = function(opt) {
  ...
  function gulpFn(/*...args*/) {
    ...
    if (isIstanbul) {
      inst._command += escapeBs(istanbulPath) +
        ' cover ' + escapeBs(gulpPath) +
...
function findCoveragePath() {
  var found = fined({ path: 'package', extensions: '.json', findUp: true });
  if (found) {
    return path.join(found.path, '../coverage');
  }

  return path.resolve('./coverage');
}
